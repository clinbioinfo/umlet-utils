...
if (!defined($converter)){
    $logger->logdie("Could not instantiate Umlet::Python::API::ClassDiagram::Converter");
}
...
=head1 NAME
 ...
 python_api_to_umlet_class_diagram.pl - Program that parses a Python API and then creates a set of Umlet class diagrams.

=head1 SYNOPSIS

 perl util/python_api_to_umlet_class_diagram.pl --indir ~/projects/my-python-project
...
=item B<--indir>
  ...
  A directory under which all Python modules discovered will be processed and included
  in the set of Umlet class diagrams.
  Default is the current working directory.

=item B<--infile>

  A specific Python module that should be processed.
...
=item B<--logfile>
  ...
  Default is [outdir]/python_api_to_umlet_class_diagram.pl.log
...
=item B<--outdir>
  ...
  of this program.
  Default is /tmp/python_api_to_umlet_class_diagram.pl/[time]/
...
=item B<--outfile>
  ...
  Default is /tmp/python_api_to_umlet_class_diagram.pl/[time]/out.xml
...
=head1 DESCRIPTION
  ...
  This program will parse all of the Python module files it finds under the specified
  input directory or just the one Python module file if the --infile option is specified.
