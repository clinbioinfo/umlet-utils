...
B<Description:> Instantiate Umlet::Logger object

B<Parameters:> 

 %args

B<Returns:> Returns a reference to Umlet::Logger
...
sub _initConfiguration {
    ...
    my $conf = q(
    log4perl.logger                    = sub {return Umlet::Logger::_getLogLevel();}

    log4perl.appender.Logfile          = Log::Log4perl::Appender::File
    log4perl.appender.Logfile.filename = sub { return Umlet::Logger::_getLogfile(); }
...
sub getInstance {
    ...
    if (!defined($instance)){
        ...
        $instance = new Umlet::Logger(@_);

        if (!defined($instance)){
            confess "Could not instantiate Umlet::Logger";
        }
    }
...
B<Description:> Umlet::Logger class destructor
...
=head1 SYNOPSIS
 ...
 use Umlet::Logger;
 my $obj = Umlet::Logger::getInstance();
