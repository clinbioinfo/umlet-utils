...
sub getInstance {
    ...
    if (!defined($instance)){
        ...
        $instance = new Umlet::Perl::Module::File::Parser(@_);

        if (!defined($instance)){

            confess "Could not instantiate Umlet::Perl::Module::File::Parser";
        }
    }
    return $instance;
}
...
=head1 SYNOPSIS
 ...
 my $parser = new Umlet::Perl::Module::File::Parser(infile=>$infile);
 $parser->getLookup();
