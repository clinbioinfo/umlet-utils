package Umlet::JavaScript::Class::Regular::File::Writer;
...
use constant DEFAULT_JAVASCRIPT_CLASS_FILE => "$FindBin::Bin/../template/javascript_class_file_tmpl.tt";
...
sub getInstance {
    ...
    if (!defined($instance)){
        ...
        $instance = new Umlet::JavaScript::Class::Regular::File::Writer(@_);

        if (!defined($instance)){

            confess "Could not instantiate Umlet::JavaScript::Class::Regular::File::Writer";
        }
    }
    return $instance;
}
...
sub _derive_getters_and_setters {
    ...
    if (( exists $class_lookup->{private_data_members_list}) && 
        ...
        $self->{_logger}->info("Looks like there are no private data members for class '$self->{_javascript_namespace}' so will not attempt to derive setter and getter functions");
    }
}
...
=head1 NAME
 ...
 Umlet::JavaScript::Class::Regular::File::Writer
...
=head1 SYNOPSIS
 ...
 my $manager = Umlet::JavaScript::Class::Regular::File::Writer::getInstance();
